...
probe, status also returns `direct_capture_verification` with
`source: "permissions_grant"`. That object is a historical observation; it
does not turn the read-only status call into a claim about current
